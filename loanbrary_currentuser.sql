...
CREATE TABLE `currentuser` (
  ...
  KEY `currentuser_user_fk_idx` (`id_user`),
  CONSTRAINT `currentuser_user_fk` FOREIGN KEY (`id_user`) REFERENCES `user_person` (`id_user`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci COMMENT='Table that stores the information from the current user logged into the system.';
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `currentuser_BEFORE_INSERT` BEFORE INSERT ON `currentuser` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `currentuser_BEFORE_UPDATE` BEFORE UPDATE ON `currentuser` FOR EACH ROW BEGIN
...
-- Dump completed on 2021-05-28 20:09:56
