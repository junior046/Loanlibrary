...
CREATE TABLE `loan` (
  ...
  PRIMARY KEY (`id_loan`),
  KEY `loan_item_fk_idx` (`id_item`),
  KEY `loan_person_fk_idx` (`id_person`),
  CONSTRAINT `loan_item_fk` FOREIGN KEY (`id_item`) REFERENCES `item` (`id_item`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `loan_person_fk` FOREIGN KEY (`id_person`) REFERENCES `person` (`id_person`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci COMMENT='Table that stores the information from the loans registered in the system.';
...
-- Dump completed on 2021-05-28 20:09:46
